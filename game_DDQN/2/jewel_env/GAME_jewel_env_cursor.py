...
class JewelCursorEnv(gym.Env):
    ...
    def _get_obs(self):
        """
        回傳 obs shape: (9, 9, 6)
        obs[0~6]: 各種寶石類型 (1~6 + 8)，每層是 one-hot mask
        obs[7]: 牆壁位置 (==7)
        obs[8]: 游標位置 (單點 mask)
        """
        gem_ids = [1, 2, 3, 4, 5, 6, 8]  # 7 種寶石 + 空格 (8)
        H, W = self.board.shape
        obs = np.zeros((9, H, W), dtype=np.float32)

        # obs[0~6]: one-hot for gem types
        for i, g in enumerate(gem_ids):
            obs[i] = (self.board == g).astype(np.float32)

        # obs[7]: wall mask
        obs[7] = (self.board == 7).astype(np.float32)

        # obs[8]: cursor position
        if hasattr(self, "cursor_row") and hasattr(self, "cursor_col"):
            obs[8, self.cursor_row, self.cursor_col] = 1.0
    ...
    def step(self, action):
        ...
        # === 處理動作（使用游標邏輯）===
        self.apply_cursor_action(action, reward_components)
        self.last_action_coord = (self.cursor_row, self.cursor_col)

        # === 處理 upload 行為 ===
        if self.upload_timer <= 0 or action == 6:
            self.upload_row()
        else:
            self.upload_timer -= 1
        ...
        # ==== 計算 reward ====
        if self.reward_mode == "simple":
            # reward_components["shaping_reward"] = self._get_shaping_reward()
            reward_components["cleared"] = 0.1 * cleared
            # if self.no_clear_steps > 20:
            #     reward_components["no_clear_penalty"] = -0.3
            if self.top_block_timer >= GAMEOVER_T:
                reward_components["gameover"] = -0.2
                done = True
        ...
        for key, val in reward_components.items():
            self.reward_sum_components.setdefault(key, 0.0)
    ...
    def apply_cursor_action(self, action, reward_components):
        if action == 0:
            return  # 不做事
        elif action == 1:
            ...
            self.cursor_col = min(COLS - 2, self.cursor_col + 1)
        elif action == 5:
            r, c = self.cursor_row, self.cursor_col
            if self.board[r][c] != WALL and self.board[r][c + 1] != WALL:
    ...
    def _get_shaping_reward(self):
        """
        鼓勵滑鼠游標靠近相同寶石、形成潛在 2 消、3 消的機會。
        """
        if not hasattr(self, "cursor_row") or not hasattr(self, "cursor_col"):
            return 0.0

        r0, c0 = self.cursor_row, self.cursor_col
        target = self.board[r0, c0]
        ...
        for dr, dc in [(-1,0), (1,0), (0,-1), (0,1)]:
            r, c = r0 + dr, c0 + dc
            if 0 <= r < 9 and 0 <= c < 6 and self.board[r, c] == target:
                reward += 0.2  # 一個相鄰的同類型寶石給 0.2 分
        ...
        for ep in range(10):
            ...
            while not done:
                ...
                action_desc = {
                    ...
                    3: "⬅️ Move Left",
                    4: "➡️ Move Right",
                    5: f"🔁 Swap at ({env.cursor_row},{env.cursor_col})",
                    6: "⬆️ Upload"
            ...
            print(f"✅ Episode {ep + 1} ended. Reward: {total_reward:.2f}, "
                  f"Cleared: {info['cleared']}, Combo: {info['combo']}")
